...
export function normalizeNotificationHtml(rawHtml: string) {
  return sanitizeNotificationHtmlString(rawHtml);
}
...
export function notificationPlainTextFromHtml(value: string | undefined) {
  if (!value) return "";

  const sanitized = sanitizeNotificationHtmlString(value);
  ...
  return normalizeWhitespace(normalizeText(container.textContent ?? ""));
}
